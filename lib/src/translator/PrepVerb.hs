...
main = do
  ...
  let pverbs = [v | v <- funs, let vs = parts v, length vs > 2, head (last vs) == 'V']
  ...
  let linPVerb v = case parts v of 
        [w,p,"V"]  | hasCat p "Adv"  && hasCat w "V"   -> advV (w ++ "_V "  ++ mkFun p "Adv") 
        [w,p,"V"]  | hasCat p "Adv"  && hasCat w "V2"  -> advV (v2V w ++ mkFun p "Adv") 
        [w,p,"V2"] | hasCat p "Prep" && hasCat w "V"   -> "prepV2 " ++ w ++ "_V "  ++ mkFun p "Prep" 
        [w,p,"V2"] | hasCat p "Adv"  && hasCat w "V"   -> "mkV2 (" ++ advV (w ++ "_V "  ++ mkFun p "Adv") ++ " )"  
        [w,p,"V2"] | hasCat p "Prep" && hasCat w "V2"  -> "prepV2 " ++ v2V w       ++ mkFun p "Prep"  
        [w,p,"V2"] | hasCat p "Adv"  && hasCat w "V2"  -> "mkV2 (" ++ advV (v2V w       ++ mkFun p "Adv") ++ " )"  
        [w,p,q,"V2"] | hasCat p "Adv"  && hasCat q "Prep"  && hasCat w "V"   -> "prepV2 (" ++ advV (w ++ "_V " ++ mkFun p "Adv") ++" ) " ++ mkFun q "Prep"  
        [w,p,q,"V2"] | hasCat p "Adv"  && hasCat q "Prep"  && hasCat w "V2"  -> "prepV2 (" ++ advV (v2V w ++ mkFun p "Adv") ++ " ) " ++ mkFun q "Prep"  
        _ -> "variants {}"

  writeFile "PrepVerbs.gf" $ unlines ["lin " ++ p ++ " = " ++ linPVerb p ++ " ; -- guess-p-verb" | p <- pverbs]
...
mkPrepVerbDict lang = do
  ...
  let rules = [s ++ "oper " ++ p ++ " : " ++ c ++ " = " ++ r | (p,c,(s,r)) <- preps ++ advs ++ verbs ++ verb2]
  
  let preamble = [
        "concrete PrepVerbs" ++ lang ++ " of Dictionary = Cat" ++ lang ++ " ** ",
