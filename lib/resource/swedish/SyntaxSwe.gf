...
oper
  verbSForm : Verbum -> Voice -> SForm -> {fin,inf : Str} = \se,vo,sf -> 
     let
       ...
       hasett : Voice -> Str = \v -> "ha" ++ sett v

     in case sf of {
       VIndic t       Simul => simple   (VF (tense t vo)) ; 
       VIndic Present Anter => compound "har" (sett vo) ;
       VIndic Past    Anter => compound "hade" (sett vo) ;
       VFut     Simul       => compound "ska" (see vo) ; 
       VFut     Anter       => compound "ska" (hasett vo) ; 
       VCondit  Simul       => compound "skulle" (see vo) ;
       VCondit  Anter       => compound "skulle" (hasett vo) ; 
       VImperat             => simple   (VF Imper) ; --- no passive
       VInfinit Simul       => simple   (VI (Inf vo)) ;
       VInfinit Anter       => compound "ha" (sett vo)
     } ;
...
-- on semantic grounds.
  ...
  predAdjective : Adjective -> VerbGroup = \arg ->
    vara (\\g,n => arg.s ! mkAdjForm Indef n g NoMasc ! Nom) ;
...
-- Sentence-complement verbs take sentences as complements.
  ...
  complSentVerb : SentenceVerb -> Sentence -> VerbGroup = \se,duler ->
    useVerb se (\\_,_ => se.s1 ++ optStr "att" ++ duler.s ! Main) ;
